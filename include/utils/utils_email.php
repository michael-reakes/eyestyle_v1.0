...
class utils_email {
	...
	var $debug_status = "no";			// "yes" | "no" | "halt"
	var $charset = "ISO-8859-1";
	var $mail_subject = "No subject";
	var $mail_from = "Anonymous <fake@mail.com>";
	var $mail_from_smtp = "<\"Anonymous\":<fake@mail.com>>";
	var $mail_to;
	var $mail_to_smtp;
	var $mail_cc;
	var $mail_bcc;
	...
	/**
	 * void set_from_smtp(string mail_from, [string name])
	 */
	function set_from_smtp($mail_from, $name = "") {
		if ($this->validate_mail($mail_from)){
			$this->mail_from_smtp = !empty($name) ? "\"$name\":<$mail_from>" : $mail_from;
		}
		else {
			$this->mail_from = "\"Anonymous\":<fake@mail.com>";
		}
	...
	/**
	 * bool set_to_smtp(string mail_to, [string name])
	 */
	function set_to_smtp($mail_to, $name = ""){
		if ($this->validate_mail($mail_to)){
			$this->mail_to_smtp = !empty($name) ? "\"$name\":<$mail_to>" : $mail_to;
			return true;
		}
		return false;
	}
	...
	function new_mail($from = "", $to = "", $subject = "", $text = "", $html = ""){
		...
		if (is_array($from)){
			...
			$this->set_from_smtp($from[0],$from[1]);
			$this->set_return_path($from[0]);
		}
		else {
			$this->set_from($from);
			$this->set_from_smtp($from);
			$this->set_return_path($from);
		}

		if (is_array($to)){
			$this->set_to($to[0],$to[1]);
			$this->set_to_smtp($to[0],$to[1]);
		}
		else {
			$this->set_to($to);
			$this->set_to_smtp($to);
	...
	function send_smtp(){
		...
		$smtp = new utils_smtp();
		$smtp->host_name="smtp.tpg.com.au";
		$smtp->debug = 1;
		$headers = 'To: '.$this->mail_to.BR;
		$headers .= 'Subject: '.$this->mail_subject.BR;
		$headers .= $this->mail_header;
		return $smtp->send_message($this->mail_from_smtp, array($this->mail_to_smtp), $headers, $this->mail_body);
	}
	...
	function build_header($content_type){
		...
		$this->mail_header .= "X-Mailer: PHP/". phpversion() . BR;
